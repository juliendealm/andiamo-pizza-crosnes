...
function save(){try{localStorage.setItem('andiamo-v2-cart',JSON.stringify(cart))}catch{toast('Panier conservé uniquement pour cette visite.')}renderCart()}
...
function render(){
 ...
 $('#products').innerHTML=all.slice(0,limit).map(p=>{const sizes=Object.keys(p.prices),first=sizes[0];return `<article class="product" data-id="${p.id}" data-size="${esc(first)}">${p.image?`<div class="photo"><img src="${p.image}" alt="${esc(p.name)}" loading="lazy" decoding="async" width="800" height="600"></div>`:''}<div class="product-body"><span class="eyebrow">${esc(p.category)}${p.base?' · '+p.base:''}</span><h3>${esc(p.name)}</h3>${p.description?`<p>${esc(p.description)}</p>`:''}${p.included?`<p class="included">${esc(p.included)}</p>`:''}${sizes.length>1?`<div class="sizes" aria-label="Format de ${esc(p.name)}">${sizes.map((s,i)=>`<button aria-pressed="${!i}" class="${i?'':'active'}" data-size="${esc(s)}">${esc(s)}</button>`).join('')}</div>`:''}<div class="product-bottom"><span class="price">${money(p.prices[first])}</span><button class="${p.count||p.drink||p.taco?'pill':'circle'} add" data-add="${p.id}" aria-label="${p.count||p.drink||p.taco?'Composer':'Ajouter'} ${esc(p.name)}">${p.count||p.drink||p.taco?'Choisir':'+'}</button></div></div></article>`}).join('')||'<p>Aucun résultat. Essayez un autre nom ou ingrédient.</p>';
 $('#products').querySelectorAll('img').forEach(img=>{const ready=async()=>{try{await img.decode()}catch{}if(img.naturalWidth)img.parentElement.classList.add('ready')};img.addEventListener('load',ready);img.addEventListener('error',()=>{img.parentElement.hidden=true},{once:true});if(img.complete)ready()});
}
...
document.addEventListener('click',e=>{
 ...
 const size=e.target.closest('.sizes button');if(size){const card=size.closest('.product'),p=catalog.find(p=>p.id===card.dataset.id);card.dataset.size=size.dataset.size;card.querySelectorAll('.sizes button').forEach(b=>{b.classList.toggle('active',b===size);b.setAttribute('aria-pressed',String(b===size))});card.querySelector('.price').textContent=money(p.prices[size.dataset.size])}
 ...
 const service=e.target.closest('[data-mode]');if(service){mode=service.dataset.mode;renderCart()}
 const offer=e.target.closest('[data-mode-offer]');if(offer){mode=offer.dataset.modeOffer;active='Pizzas';$('#search').value='';limit=12;tabs();render();renderCart();$('#carte').scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});toast(mode==='pickup'?'À emporter sélectionné':'Livraison sélectionnée')}
...
$('#search').addEventListener('input',()=>{active='Tous';limit=12;tabs();render()});$('#more').addEventListener('click',()=>{limit+=12;render()});
