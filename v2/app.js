...
function renderMenus(){
 $('#menus').innerHTML=catalog.filter(p=>p.category==='Menus').map(p=>`<article class="menu-card"><div class="photo menu-photo"><img src="${esc(p.image)}" alt="Exemple de présentation du ${esc(p.name)}" loading="lazy" decoding="async" width="800" height="800"><span class="menu-photo-caption">Suggestion de présentation</span></div><div class="menu-card-body"><span class="eyebrow">${esc(p.label)}</span><div class="menu-card-title"><h3>${esc(p.name).toUpperCase()}</h3><strong class="price">${money(p.prices.Menu)}</strong></div><p>${esc(p.description)}</p><button class="pill gold" data-add="${p.id}">${p.count?'Composer mon menu':'Ajouter ce menu'}</button></div></article>`).join('');
 $('#menus').querySelectorAll('img').forEach(img=>{const ready=async()=>{try{await img.decode()}catch{}if(img.naturalWidth)img.parentElement.classList.add('ready')};img.addEventListener('load',ready,{once:true});img.addEventListener('error',()=>{img.parentElement.hidden=true},{once:true});if(img.complete)ready()});
}
